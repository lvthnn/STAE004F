...
tau_post <- sapply(tau_vec, function(tau) {
  # Construct Qx
  Qx <- diag(tau^-2, Q + 1)
  Qx[1, 1] <- 100^-2
  ...
  tau_post <- det(Qxy)^(-1/2) * tau^(-Q) * exp(scaling * (sigma^-2/2 * t(mu_xy) %*% t(Z) %*% y - 2 * tau))
  return(tau_post)
})
...
mcmc_simulation <- function(S, burnin) {
  ...
  theta_post <- function(theta) { k^-1 * exp(-k^-1 * exp(theta) + theta) }
  ...
  for (s in 2:S) {
    ...
    alpha <- min(1, theta_post(theta_curr) / theta_post(theta_mc[s - 1]))
    u <- runif(1)
...
tau_vec   <- seq(0, max(range(chains[[1]][,"tau"])), by = delta)
tau_prior <- dexp(tau_vec, k^-1)

pdf("tau_densities.pdf")
plot(tau_vec, tau_prior, lwd = 1.5, type = "l", xlab = expression(tau), ylab = "Density")
lines(density(chains[[1]][,"tau"]), lwd = 1.5, lty = 2)
legend("topright", inset = c(0.025, 0.025), bty = "n", lty = 1:2, legend = c("Prior", "Posterior"))
dev.off()
