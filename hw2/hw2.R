...
## part (a)
lm.kappa <- lm(Q ~ I(r_mat^(2 / 3) * A * S^(1 / 2)), data = dat)
kappa <- unname(coef(lm.kappa)[2])^-1 # estimate Manning roughness coef.

pdf("1a_Q_vs_h.pdf", width = 12)
par(mfrow = c(1, 2))
plot(Q ~ h, data = dat)
with(dat, lines(h, kappa^-1 * r_mat^(2 / 3) * A * S^(1 / 2), lty = 2))
with(dat, plot(kappa^-1 * r_mat^(2 / 3) * A * S^(1 / 2), Q, xlab = TeX("$\\hat{Q} = \\kappa^{-1} R^{2/3} A S^{1/2}$")))
abline(a = 0, b = 1, lty = 2)
dev.off()

## part (b)

dat$y <- with(dat, log(Q) - 2 / 3 * log(r_mat) - log(A) - 1 / 2 * log(S))
...
r_mat <- R22 <- R21 <- matrix(0, n, n)

for (i in 1:n) {
  for (j in 1:n) {
    r_mat[i, j] <- (1 + (sqrt(20) * abs(h[i] - h[j])) / phi_u +
      (20 * (h[i] - h[j])^2) / (3 * phi_u^2)) * exp(-(sqrt(20) *
      abs(h[i] - h[j])) / phi_u)
    r21[i, j] <- (1 + (sqrt(20) * abs(h[i] - h_p[j])) / phi_u +
      (20 * (h[i] - h_p[j])^2) / (3 * phi_u^2)) * exp(-(sqrt(20) *
      abs(h[i] - h_p[j])) / phi_u)
...
log_tau_post <- function(s_u, s_e) {
  cov_y <- s_eta^2 * matrix(1, n, n) + s_u^2 * r_mat + s_e^2 * diag(n)
  mu_y <- rep(mu_eta, n)

  prior <- -mu_su^-1 * s_u - mu_se^-1 * s_e
  like <- -1 / 2 * log(det(cov_y)) - 1 / 2 * t(y - mu_y) %*%
    solve(cov_y) %*% (y - mu_y)
  res <- prior + like
...
conf_int <- sum(tau_post_grid * darea)

tau_post_grid <- conf_int^-1 * tau_post_grid

surf3D(su_mesh, se_mesh, tau_post_grid,
  box = TRUE,
...
log_post <- function(theta) {
  ...
  cov_y <- s_eta^2 * matrix(1, n, n) + exp(theta_u)^2 * r_mat +
    exp(theta_e)^2 * diag(n)

  mu_y <- rep(mu_eta, n)

  prior <- -mu_su^-1 * exp(theta_u) + theta_u - mu_se^-1 *
    exp(theta_e) + theta_e
...
mcmc_simulation <- function(nsamps, burnin) {
  nits <- nsamps + burnin
  x_mc <- matrix(nrow = nits, ncol = n + m + 3)
  theta_mc <- matrix(nrow = nits, ncol = 2)

  colnames(x_mc) <- c(
    "eta", paste0("u", 1:n),
    paste0("u", 1:m, "p"), "s_u", "s_e"
  )
  ...
  for (s in 2:nits) {
    ...
    cov_x <- bdiag(s_eta^2, s_u^2 * r_mat)
    ...
    b_mat <- solve(s_eta^2 * matrix(1, n, n) + s_u^2 * r_mat + s_e^2 * diag(n))

    mu_xy <- mu_x + a_mat %*% b_mat %*% (y - mu_eta * rep(1, n))
    cov_xy <- cov_x - a_mat %*% b_mat %*% t(a_mat)

    x_c <- mvrnorm(1, mu_xy, cov_xy)

    # sample from posterior predictive density
    mu_p <- r21 %*% solve(r_mat) %*% x_c[-1]
    cov_p <- R22 - r21 %*% solve(r_mat) %*% t(r21)
    y_c <- mvrnorm(1, mu_p, cov_p) + mvrnorm(1, rep(0, m), s_e^2 * diag(m))

    x_mc[s, ] <- c(x_c, y_c, tau_c)
  }

  return(x_mc = x_mc[(burnin + 1):nits, ])
}
...
save(mcmc_chains, file = "chains.Rdata")

gelman.diag(mcmc_chains)

## part (h)
sum.chains <- summary(mcmc_chains)
...
plot(
  h, eta + uh,
  type = "l", ylim = range(eta + uh_l - 0.025, eta + uh_u + 0.025), lwd = 2
)
...
plot(h, eta + uh,
  xlim = c(0.000, 0.13), ylim = range(4, 5), col = "blue",
  type = "l", ylab = expression(eta + u(h[i]))
)
...
legend(
  "bottomright",
  bty = "n", legend = c("Predicted", "Fitted"),
